...
export const formDataTransformer = {
  ...
  parse: (formData) => {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (value === '' && (key.includes('_from_bin') || key.includes('_qty_out'))) {
  ...
  toSchema: (parsed) => {
    ...
    return {
      _trx_header: {
        ...
        _market_id: parsed._trx_header?._market_id ? parseInt(parsed._trx_header._market_id) : null,
        _trx_type_id: parsed._trx_header?._trx_type_id ? parseInt(parsed._trx_header._trx_type_id) : null,
