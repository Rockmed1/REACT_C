...
export default function AddItemClassForm({ onCloseModal }) {
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      const result = await createItemClass(null, formData);
    ...
    onMutate: async (newItemClass) => {
      ...
      queryClient.setQueryData(["itemClass", "all"], (old = []) => [
        ...old,
        {
          ...
          nameField: newItemClass.nameField,
          descField: newItemClass.descField,
          optimistic: true,
        },
      ]);
    ...
    onSuccess: (result, variables) => {
      ...
      toast.success(
        `Item Class ${variables.nameField} was created successfully!`,
      );
      form.reset();
      onCloseModal?.();
    },
